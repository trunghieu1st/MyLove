...
<html>
    ...
    <head>
        ...
        body {
            margin: 0;
            overflow: hidden;
            /* ẩn scroll khi avatar chạy lung tung */
            background: #000;
            /* nền cho dễ nhìn */
        }
        ...
        /* Ngừng xoay container để rect không phình */
        .music-avatar-ny,
        .music-avatar-my {
            animation: none !important;
        }

        /* Chỉ xoay ảnh bên trong */
        .music-avatar-ny img,
        .music-avatar-my img {
            animation: spin 8s linear infinite;
        }
    ...
    <body>
        ...
        <div class="music-avatar-ny" id="avatarBtn-ny">
            <img src="my.jpg" alt="Ny của mình">
        </div>

        <!-- Avatar m quay tròn -->
        <div class="music-avatar-my" id="avatarBtn-my">
            <img src="ny.jpg" alt="Ny của mình">
        </div>
        ...
        let dxMy = 3, dyMy = 2; // cho khác tốc độ để vui hơn 😎
        ...
        <script>
        // Lấy hình tròn (tâm & bán kính) từ state + kích thước thật
        function getCircle(el, x, y) {
            const r = el.offsetWidth / 2; // avatar là vuông -> r = width/2
            return { x: x + r, y: y + r, r };
        }

        // Phản xạ vận tốc v qua pháp tuyến n (đã chuẩn hoá)
        ...
        let collided = false; // chốt để không spam collision mỗi frame
        ...
        function checkCollisionLoop() {
            ...
            // DÙNG state xNy,yNy,xMy,yMy (đã có ở script move) -> KHÔNG dùng getBoundingClientRect
            ...
            if (dist <= sumR) {
                if (!collided) {
                    // Nổ tim đúng điểm chạm (giữa hai tâm)
                    ...
                    // Đẩy tách ra để không dính
                    const nx = dist === 0 ? 1 : dx / dist; // phòng TH trùng tâm hiếm gặp
                    const ny = dist === 0 ? 0 : dy / dist;
                    const overlap = sumR - dist + 0.1; // +epsilon để chắc chắn rời nhau
                    xNy -= nx * overlap / 2;
                    yNy -= ny * overlap / 2;
                    xMy += nx * overlap / 2;
                    yMy += ny * overlap / 2;

                    // Bật nẩy: phản xạ vận tốc theo pháp tuyến va chạm
                    [dxNy, dyNy] = reflect(dxNy, dyNy, nx, ny);
                    [dxMy, dyMy] = reflect(dxMy, dyMy, -nx, -ny);

                    collided = true; // khoá cho đến khi rời nhau
                }
            } else {
                collided = false;
            }
        ...
        // ================= Text tuần tự =================
        const texts = [
            "Gặp một người được coi là định mệnh",
            "Tim mình lỡ thuộc về bạn mất rồi",
            "Trọn đời này chỉ mong được có bạn bên cạnh",
            "LOVE HẰNG❤️"
        ];
        let index = 0;
